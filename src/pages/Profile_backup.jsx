...
const Profile = () => {
    ...
    const handleDeleteAccount = async () => {
        const confirmDelete = window.confirm(t('deletionConfirmation'));
        if (!confirmDelete) return;

        const loadingToast = toast.loading(t('deletingAccount'));
        try {
    ...
    const handleAvatarUpload = async (e) => {
        ...
        const loadingToast = toast.loading(t('uploadingAvatar'));
    ...
    const currencies = ["USD ($)", "EUR (â‚¬)", "GBP (Â£)", "INR (â‚¹)", "JPY (Â¥)", "CNY (Â¥)", "AUD (A$)", "CAD (C$)"];
            ...
            <div className="max-w-4xl mx-auto w-full space-y-8 pb-12">
                ...
                <div className="bg-card border border-border p-6 rounded-[32px] shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Account Type */}
                        <div className="flex items-center gap-4 p-5 bg-gradient-to-br from-purple-500/5 to-purple-500/10 rounded-2xl border border-purple-500/10 hover:border-purple-500/20 transition-all">
                            <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-purple-500/10 shadow-inner">
                                <Crown className="w-6 h-6 text-purple-600" />
                            </div>
                            <div className="flex-1">
                                <p className="text-xs font-semibold text-subtext uppercase tracking-wide mb-1">{t('accountType')}</p>
                                <p className="text-base font-bold text-maintext capitalize">{user.role || t('user')}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 p-5 bg-gradient-to-br from-blue-500/5 to-blue-500/10 rounded-2xl border border-blue-500/10 hover:border-blue-500/20 transition-all">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex items-center gap-3">
                                    <p className={`text-base font-bold ${userSettings.emailNotif ? 'text-green-600' : 'text-gray-500'}`}>
                                        {userSettings.emailNotif ? t('enabled') : t('disabled')}
                                    </p>
                                    {/* Toggle Switch */}
                                    <button
                                        onClick={() => toggleSetting('emailNotif')}
                                        className={`relative w-12 h-6 rounded-full transition-all duration-300 shadow-inner ${userSettings.emailNotif ? 'bg-green-500' : 'bg-gray-300'
                                            }`}
                                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                    <div className="bg-card border border-border rounded-[32px] p-8 space-y-8">
                        ...
                        <div className="space-y-6 pt-6 border-t border-border">
                            <h2 className="text-xl font-bold text-maintext flex items-center gap-2"><Bell className="w-5 h-5 text-blue-500" />{t('notifications')}</h2>
                            <div className="space-y-4">
                                {['emailNotif', 'pushNotif'].map(k => (
                                    <div key={k} className="flex items-center justify-between">
                                        <div><p className="text-sm font-bold text-maintext">{k === 'emailNotif' ? t('emailNotifications') : t('pushNotifications')}</p><p className="text-xs text-subtext">{t('receiveUpdates')}</p></div>
                    ...
                    <div className="bg-card border border-border rounded-[32px] p-8 flex flex-col justify-between">
                        ...
                        <div className="space-y-3 pt-8">
                            <button onClick={handleDeleteAccount} className="w-full py-4 bg-red-500/5 text-red-600 border border-red-500/10 rounded-2xl font-bold text-sm hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-2"><Trash2 className="w-4 h-4" />{t('deleteAccount')}</button>
                        </div>
                    </div>
                </div>
            </div>
